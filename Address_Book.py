...
def list_contacts():
    global cursor
    cursor.execute('SELECT NAME FROM mydb.CONTACTS')
...
def submit_record():
  ...
  if name=='' or email=='' or phone=='' or address=='':
      ...
      cursor.execute(
      "INSERT INTO mydb.CONTACTS (NAME, EMAIL, PHONE_NUMBER, ADDRESS) VALUES (%s,%s,%s,%s)", (name, email, phone, address))
      connector.commit()
...
def search():
  ...
  if query != '':
   listbox.delete(0, END)
   cursor.execute('SELECT * FROM mydb.CONTACTS WHERE NAME LIKE %s', ('%'+query+'%', ))
...
def view_record(): 
  ...
  queryString = "SELECT * FROM mydb.CONTACTS WHERE NAME='" + listbox.get(ACTIVE) + "'"
...
def delete_record(): 
    ...
    cursor.execute('DELETE FROM mydb.CONTACTS WHERE NAME=%s', (listbox.get(ACTIVE)))
    connector.commit()
...
def delete_all_records(): 
    cursor.execute('DELETE FROM mydb.CONTACTS')
    connector.commit()
